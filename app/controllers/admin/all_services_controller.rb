...
module Admin
  class AllServicesController < BaseController
    ...
    def index
      ...
      load_created_service_follow_up
      load_services
        ...
        format.turbo_stream do
          render turbo_stream: [
            turbo_stream.append(
              ...
              collection: @services,
              as: :service,
              locals: { query: @query, row_offset: @row_offset, show_partner: true }
            ),
        ...
        respond_to do |format|
          format.html do
            redirect_to admin_services_path(
              ...
              prompt_create_task: true
            ), notice: 'Tạo dịch vụ thành công.'
          end
    ...
    def load_created_service_follow_up
      @created_service = Service.includes(:partner).find_by(id: params[:created_service_id])
      return unless @created_service.present?

      @show_create_task_prompt = ActiveModel::Type::Boolean.new.cast(params[:prompt_create_task])
      @open_task_modal = ActiveModel::Type::Boolean.new.cast(params[:open_task_form])
      @task = @created_service.tasks.build if @open_task_modal
      @member_options = member_options if @open_task_modal
    end
